...
// to a POSIX shared memory ring buffer. Multiple consumers (video_saver, IMF, etc.)
// can read the demosaiced frames without per-consumer demosaic overhead.
//
// Usage:
//   ShmFrameProducer producer;
//   if (producer.init("camera_lucid_wrist_left", 1440, 1080, NPPI_BAYER_RGGB, 16)) {
//     // In frame grab loop:
...
class ShmFrameProducer {
 public:
  ...
  /// Initialize the producer: allocate GPU buffers, create shared memory ring buffer.
  /// @param camera_name  Camera identifier (used in shm name: "nmgc_cam_ring_{camera_name}").
  /// @param width        Frame width in pixels.
  /// @param height       Frame height in pixels.
  /// @param bayer_pattern NPP Bayer pattern (e.g., NPPI_BAYER_RGGB).
  /// @param num_slots    Number of ring buffer slots.
  /// @return true on success.
  bool init(const std::string& camera_name, size_t width, size_t height, NppiBayerGridPosition bayer_pattern,
            uint32_t num_slots) {
    if (initialized_) {
    ...
    cudaError_t err = cudaMallocPitch(&d_bayer_, &d_bayer_pitch_, width, height);
    if (err != cudaSuccess) {
      fprintf(stderr, "ShmFrameProducer: cudaMallocPitch for bayer failed: %s\n", cudaGetErrorString(err));
      return false;
    }
    ...
    if (!producer_.create(shm_name, static_cast<uint32_t>(width), static_cast<uint32_t>(height), "rgb8", 3,
                          num_slots)) {
  ...
  /// Write a Bayer frame: upload to GPU, demosaic to RGB, download, write to ring buffer.
  /// @param raw_bayer    Pointer to raw Bayer data (host memory).
  /// @param bayer_size   Size of Bayer data in bytes (must equal width * height).
  /// @param timestamp_ns Frame timestamp in nanoseconds.
  /// @param frame_id     Camera-assigned frame ID.
  /// @return true on success.
  bool write(const uint8_t* raw_bayer, size_t bayer_size, uint64_t timestamp_ns, uint64_t frame_id) {
    if (!initialized_) {
      return false;
    }

    // Upload Bayer to GPU
    cudaError_t err = cudaMemcpy2D(d_bayer_, d_bayer_pitch_, raw_bayer, width_, width_, height_, cudaMemcpyHostToDevice);
    if (err != cudaSuccess) {
    ...
    NppiRect src_roi = {0, 0, static_cast<int>(width_), static_cast<int>(height_)};
    NppStatus npp_status = nppiCFAToRGB_8u_C1C3R_Ctx(
        static_cast<const Npp8u*>(d_bayer_), static_cast<int>(d_bayer_pitch_), src_size, src_roi,
        static_cast<Npp8u*>(d_rgb_), static_cast<int>(d_rgb_pitch_), bayer_pattern_, NPPI_INTER_UNDEFINED, npp_ctx_);
    ...
    err = cudaMemcpy2D(rgb_staging_.data(), width_ * 3, d_rgb_, d_rgb_pitch_, width_ * 3, height_,
    ...
    return producer_.write_frame(rgb_staging_.data(), rgb_staging_.size(), timestamp_ns, frame_id);
  }
 ...
 private:
  bool initNppStreamContext() {
    ...
    cudaDeviceGetAttribute(&npp_ctx_.nCudaDevAttrComputeCapabilityMajor, cudaDevAttrComputeCapabilityMajor, device_id);
    cudaDeviceGetAttribute(&npp_ctx_.nCudaDevAttrComputeCapabilityMinor, cudaDevAttrComputeCapabilityMinor, device_id);
